...
-- Read a message and enforce a timeout. The response will be in a Maybe indicating whether it timed out or
-- not. If it didn't time out, then the Either will be an indication if the packet it found parsed or not.
...
-- We start our interaction with a new websocket here, to do the basic HELO exchange
...
-- Second state of a new websocket exchange, HELO worked, now its on to doing the AUTH before we transition
-- to message sending/receiving mode
  ...
  where
    ...
    process (Right auth@(ExistingAuth deviceID _)) cdata
      | W.verifyAuth "secret" auth = do
        ...
        -- Atomically determine if this DeviceID already exists in the server states client map. If it does,
        -- return an IO op that will kill the dupe client. Either way we update the client map so that the
    ...
    -- Define a safe clean-up that ensures if the rest of this clients interaction goes bad, we will ALWAYS
    -- remove this client from the client mapping. Note that we have to check to ensure we don't delete this
    -- deviceId if the threadId doesn't match our own threadId (ie, maybe we've been killed).
...
-- Final transition to message send/receive mode. This runs until the client does something bad which will
...
messagingApplication state uuid (ping, _, conn) = forever $ do
  ...
  -- We have a nested case here of an Either inside a Maybe. The Maybe indicates whether or not we timed out
  -- attempting to read data. The Either indicates if the message parsed or not. We only accept Outgoing/Ping
  -- messages here, all else results in dropping the connection.
  case pmsg of
    Just (Right packet@(Outgoing s mid _)) -> do
      print packet

      -- Run an STM atomic operation that reads the list of available relays and attempts to put the message
      -- on a bounded queue (tbqueue) of the first relay available. If no relay is available, we NACK the
      -- message. If the relay's queue is full, this blocks until either the relay list is updated (relay is
      -- removed/added) or the queue has space.
      join . atomically $ do
